...
namespace HealthCareSmart.MVC.Controllers
{
    public class AppointmentsController : Controller
    {
        ...
        public async Task<IActionResult> Prescription(int id)
        {
         ...
         // ✅ NEW: Payment action
        [HttpPost]
        public async Task<IActionResult> Pay(int billingId, decimal amount)
        {
            var token = HttpContext.Session.GetString("JwtToken");
            if (string.IsNullOrEmpty(token))
                return RedirectToAction("Login", "Account");
            ...
            var response = await client.PostAsync("api/billings/pay", content);
if (response.IsSuccessStatusCode)
            {
            ...
            else
            {
                ...
                TempData["ErrorMessage"] = $"Payment failed: {error}";
            }
