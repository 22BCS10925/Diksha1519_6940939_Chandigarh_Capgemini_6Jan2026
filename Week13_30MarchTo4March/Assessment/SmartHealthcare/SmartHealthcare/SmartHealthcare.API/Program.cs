...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
       ...
       // ✅ Seed bills if none exist
if (!dbContext.Bills.Any())
{
    Log.Information("Seeding sample bills...");

    dbContext.Bills.AddRange(
        new SmartHealthcare.Models.Entities.Bill
        {
            BillId = 1,
            AppointmentId = 101,
            ConsultationFee = 500m,
            MedicineCharges = 150m,
            PaymentStatus = "Paid"
        },
        new SmartHealthcare.Models.Entities.Bill
        {
        ...
        new SmartHealthcare.Models.Entities.Bill
        {
            ...
            MedicineCharges = 100m,
            PaymentStatus = "Paid"
        }
    );

    dbContext.SaveChanges();
    Log.Information("Sample bills seeded.");
}
    }
    catch (Exception ex)
    {
